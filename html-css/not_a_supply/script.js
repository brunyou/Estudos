...
document.addEventListener('DOMContentLoaded', function () {
    const palavras = ['******', 'supply','%$@!*#', '供給', 'إمداد' ]; // Adicione suas palavras aqui
    const intervalo = 500; // Meio segundo (em milissegundos)
    let indiceAtual = 0;

    function atualizarPalavra() {
        document.getElementById('palavra-container').textContent = palavras[indiceAtual];
        indiceAtual = (indiceAtual + 1) % palavras.length;
    }

    setInterval(atualizarPalavra, intervalo);
});
